...
function getNextFriday(date) {
  let friday = date;
  do {
    friday = new Date(friday.getTime() + 24 * 3600 * 1000);
...
function isDateInPeriod(date, period) {
  ...
  if (newDate - startDate >= 0 && endDate - newDate >= 0) {
    return true;
  }
  return false;
}
...
function formatDate(date) {
  ...
  if (newDate.getUTCHours() === 12) {
    res = `${newDate.getUTCMonth() + 1}/${newDate.getUTCDate()}/${newDate.getUTCFullYear()}, 12:`;
  } else {
    res = `${newDate.getUTCMonth() + 1}/${newDate.getUTCDate()}/${newDate.getUTCFullYear()}, ${newDate.getUTCHours() % 12}:`;
  }
  if (newDate.getUTCMinutes() < 10) {
    res += `0${newDate.getUTCMinutes()}:`;
  } else {
    res += `${newDate.getUTCMinutes()}:`;
  }
  if (newDate.getUTCSeconds() < 10) {
    res += `0${newDate.getUTCSeconds()} `;
  } else {
    res += `${newDate.getUTCSeconds()} `;
  }
  if (newDate.getUTCHours() < 12) {
    res += 'AM';
  } else {
...
function getWeekNumberByDate(date) {
  let curTime = new Date(date.getUTCFullYear(), 0).getTime();
  let startDay = new Date(date.getUTCFullYear(), 0).getUTCDay();
  let curDay = startDay;
  ...
  while (curTime < time) {
    ...
    curTime = new Date(curTime + 24 * 3600 * 1000).getTime();
    curDay = new Date(curTime).getUTCDay();
  }
  if (startDay === 0) {
    startDay += 7;
  }
  curDay = curDay === 0 ? 7 : curDay;
  if (startDay >= curDay || startDay === 0) {
    return count + 1;
  }
  return count;
}
...
function getNextFridayThe13th(date) {
  let curDate = new Date(date.getTime() + 24 * 3600 * 1000);
  while (!(curDate.getDay() === 5 && curDate.getDate() === 13)) {
...
function getQuarter(date) {
  const part = date.getTime() - new Date(date.getFullYear(), 0);
  const year =
    new Date(date.getFullYear(), 11, 31) - new Date(date.getFullYear(), 0);
  if (part / year < 0.25) {
    return 1;
  }
  if (part / year < 0.5) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let curDate = new Date(
    `${period.start.split('-')[1]}-${period.start.split('-')[0]}-${period.start.split('-')[2]}`
  );
  while (
    curDate.getTime() <=
    new Date(
      `${period.end.split('-')[1]}-${period.end.split('-')[0]}-${period.end.split('-')[2]}`
    ).getTime()
  ) {
    let workDay = 0;
    let weekday = 0;
    while (
      ...
      arr.push(`${day}-${month}-${curDate.getFullYear()}`);
      curDate = new Date(curDate.getTime() + 24 * 3600 * 1000);
      workDay += 1;
    }
    while (
      weekday < countOffDays &&
      curDate.getTime() <=
        new Date(
          `${period.end.split('-')[1]}-${period.end.split('-')[0]}-${period.end.split('-')[2]}`
        ).getTime()
